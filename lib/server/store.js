...
const defaultAcademyData = {
  hero: {
    ...
    interviewCtaLabel: 'Подать заявку на интервью'
  },
  intake: {
...
function getMemoryState() {
  if (!globalThis.__sparkMemoryState) {
    globalThis.__sparkMemoryState = cloneDefaultState();
  }

  return globalThis.__sparkMemoryState;
}
...
async function getState() {
  ...
  try {
    saved = await kv.get(DATA_KEY);
  } catch (error) {
    saved = getMemoryState();
  }
...
async function saveState(state) {
  globalThis.__sparkMemoryState = state;
